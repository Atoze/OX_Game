...
public class MatchTest {
    ...
    public void Match3on横並び確認テスト() throws BoardIndexOutOfBoundsException, PlayerIdException {
        ...
        board.insert(1, 0);
        assertThat(false, is(match.isRowAligned(new ScreenBoard(board), 1, 1, 3)));
        board.insert(1, 2);
    ...
    public void Match3on縦並び確認テスト() throws BoardIndexOutOfBoundsException, PlayerIdException {
        ...
        Board board = new SquaredBoard(3);
        board.insert(1, 0);
        assertThat(false, is(match.isColumnAligned(new ScreenBoard(board), 1, 6, 3)));
        board.insert(1, 3);
        assertThat(true, is(match.isColumnAligned(new ScreenBoard(board), 1, 6, 3)));

        board.insert(2, 1);
        board.insert(2, 4);
        assertThat(true, is(match.isColumnAligned(new ScreenBoard(board), 2, 7, 3)));

        board.insert(2, 5);
        board.insert(1, 8);
        assertThat(false, is(match.isColumnAligned(new ScreenBoard(board), 2, 2, 3)));
    }
    ...
    public void Match3on3斜め並び確認テスト() throws BoardIndexOutOfBoundsException, PlayerIdException {
        ...
        Board board = new SquaredBoard(3);
        board.insert(1, 0);
        assertThat(false, is(match.isDiagonalAligned(new ScreenBoard(board), 1, 4, 3)));
        board.insert(1, 8);
        assertThat(true, is(match.isDiagonalAligned(new ScreenBoard(board), 1, 4, 3)));

        board.insert(2, 2);
        board.insert(2, 4);
        assertThat(true, is(match.isDiagonalAligned(new ScreenBoard(board), 2, 6, 3)));
    }
}
